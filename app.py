...
def predict():
    try:
        ...
        if TENSORFLOW_AVAILABLE and model is not None:
            ...
            buffalo_kws = ['water_buffalo', 'bison', 'american_bison', 'yak', 'wild_yak', 'ox']
            cattle_kws = ['cow', 'bull', 'ox', 'oxcart', 'heifer', 'steer', 'calf', 'longhorn', 'jersey_cow']
            ...
            for item in decoded_preds:
                label = item[1].lower()
                prob = float(item[2])
                if any(kw in label for kw in buffalo_kws) and prob > max_prob:
                    found_buffalo = True
                    found_cattle = False
                    max_prob = prob
                elif any(kw in label for kw in cattle_kws) and not found_buffalo and prob > max_prob:
                    found_cattle = True
                    max_prob = prob
